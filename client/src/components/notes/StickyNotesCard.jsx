...
export default function StickyNotesCard() {
  ...
  return (
    <div className="rounded-3xl bg-surface/90 border border-white/10 p-5 shadow-xl backdrop-blur-xl space-y-4 hover:border-yellow-500/30 transition-all duration-300">
      {/* Header (Matching User Screenshot 3) */}
      ...
      {totalNotes === 0 && !isCreatingInline ? (
        ...
        /* Preview Notes Grid (Matching User Screenshot 3) */
        <div className="space-y-2.5">
          {previewNotes.map((note) => (
            <div
              key={note.id}
              onClick={openStickyNotes}
              className="p-3.5 rounded-2xl bg-yellow-500/10 border border-yellow-500/30 hover:border-yellow-400/50 cursor-pointer transition-all space-y-1 shadow-sm"
            >
              <div className="flex items-center justify-between">
      ...
      {/* Footer Metrics & OPEN ALL Trigger (Matching User Screenshot 3) */}
      <div className="pt-3 border-t border-white/10 flex items-center justify-between text-xs">
        ...
        <button
          onClick={openStickyNotes}
          className="text-xs font-black tracking-wider text-accent hover:text-accent-light flex items-center gap-1 transition-colors group uppercase"
        >
          <span>OPEN ALL</span>
