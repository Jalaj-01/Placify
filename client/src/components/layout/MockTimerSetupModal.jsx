    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md p-4 animate-in fade-in duration-200">
        <motion.div
          ...
          className="w-full max-w-lg rounded-3xl bg-surface/95 dark:bg-[#0c0d15] border border-accent/40 p-6 sm:p-7 shadow-2xl space-y-5 text-text-primary backdrop-blur-2xl"
        >
          {/* Header */}
          <div className="flex items-center justify-between border-b border-white/10 pb-4">
            <div className="flex items-center gap-3">
              <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-accent/20 text-accent-light border border-accent/30 shadow-md">
          ...
          <form onSubmit={handleStart} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <button
                    ...
                    className={`p-3.5 rounded-2xl border text-left transition-all flex flex-col justify-between space-y-1 ${
                      isSelected
                        ...
                        : 'border-white/10 bg-surface/40 text-text-muted hover:border-white/20 hover:bg-surface'
                    }`}
                  >
                    <span className="font-extrabold text-sm text-text-primary">{opt.label}</span>
              ...
              >
                <span className="font-extrabold text-sm text-text-primary">Custom Duration</span>
            ...
            {isCustom && (
              <div className="p-3 rounded-2xl bg-base border border-accent/40 space-y-1.5 animate-in fade-in">
                ...
                <input
                  ...
                  className="w-full bg-surface border border-white/10 rounded-xl px-3.5 py-2 text-sm font-mono font-bold text-text-primary placeholder:text-text-muted focus:outline-none focus:border-accent"
                  autoFocus
                />
              </div>
            )}
            ...
            <div className="pt-2 flex items-center justify-end gap-3">
              <button
                type="button"
                onClick={closeTimerSetup}
                className="px-4 py-2.5 rounded-xl text-xs font-bold text-text-muted hover:text-text-primary transition-colors"
              >
                Cancel
              </button>
              <button
                type="submit"
                className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-accent to-accent-light text-white font-extrabold text-xs shadow-lg shadow-accent/30 hover:opacity-95 transition-all flex items-center gap-2"
              >
